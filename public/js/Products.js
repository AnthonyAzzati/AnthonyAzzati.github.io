...
class AllProducts {
    ...
    productsData() {
        fetch("http://localhost:3000/api/cameras")
            ...
            .then (data => {
                const productHtml = data.map(camera => {
                    return `
                    <figure id="figure" class="flex flex-col rounded-md bg-white shadow-md m-4 p-4  md:flex-row md:mx-4">
                        <img src="${camera.imageUrl}" class="w-80 h-auto object-cover rounded-md" alt="${camera.name}" id="product-image">
                ...
                .join("")
                document.querySelector("#products").insertAdjacentHTML("afterbegin", productHtml);
            })
            .catch(error => {
                console.log(error);
            });
